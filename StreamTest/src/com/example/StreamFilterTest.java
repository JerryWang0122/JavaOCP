...
public class StreamFilterTest {
    ...
    public static void main(String[] args) {
        ...
        personList.stream().filter(p -> p.getCity() == City.Kaohsiung || p.getCity() == City.Tainan)
                .forEach(p -> service.sendEmail(p));
                
        //宣告Predicte過濾高雄男性成員, 使用forEach()傳簡訊給過濾後成員
        System.out.println("傳簡訊給高雄男性成員");
        Predicate<Person> criteria = p -> p.getCity() == City.Kaohsiung && p.getGender() == Gender.MALE;
        personList.stream().filter(criteria).forEach(p -> service.sendMessage(p));
